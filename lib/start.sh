...
start_command() {
	...
	if $test; then
		declare -A test_info=([command]="$command" [server_name]="$server_name" [monitor]="$monitor" [server_init_mem]="$server_init_mem" [server_max_mem]="$server_max_mem" [daemon]="$daemon" [players]="$players" [frequency]="$frequency" [test]="$test")
		test_form test_info
	fi
...
start_server() {
	...
	if $daemon; then
		exec $(which java) ${mem[@]} ${garbageCollection[@]} ${args[@]} -jar fabric-server-launch.jar --nogui
	else

		printf '%s' "$(tty_escape "2;36")"

		java ${mem[@]} ${garbageCollection[@]} ${args[@]} -jar fabric-server-launch.jar --nogui <>$pipe >/dev/null &

		sleep 3

		tail -f "${CRAFT_SERVER_DIR}/${server_name}/logs/latest.log" &
		tailpid=$!
		...
		printf '%s' "$(tty_escape 0)"

		pid

		# Do this if the server is running
		echo
		fwhip "$(form "normal" "italic" "\"${server_name}\"") Minecraft server running on port: $(form "green" "normal" "${server_port}") PID: $(form "green" "normal" "${PID}")"
		...
		if $monitor; then
			...
			sudo sed -i '' "s/_craft_path_/${craft_path}/g" $daemon_path
			sudo sed -i '' "s/_args_/-$($players && printf '%s' "p")n/g" $daemon_path
